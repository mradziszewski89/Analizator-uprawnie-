...
function renderStatsCards() {
  ...
  var cards = [
    { icon: 'webapp', cls: 'icon-webapp', value: stats.WebApplicationCount || 0, label: 'Web Applications',
      svgPath: '<rect x="2" y="3" width="20" height="14" rx="2"/><line x1="8" y1="21" x2="16" y2="21"/><line x1="12" y1="17" x2="12" y2="21"/>' },
    { icon: 'site', cls: 'icon-site', value: stats.SiteCollectionCount || 0, label: 'Site Collections',
      svgPath: '<path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/>' },
    { icon: 'web', cls: 'icon-web', value: stats.WebCount || 0, label: 'Witryny (Webs)',
    ...
    { icon: 'list', cls: 'icon-list', value: stats.ListCount || 0, label: 'Listy i biblioteki',
    ...
    { icon: 'file', cls: 'icon-file', value: (stats.ItemCount || 0) + (stats.FolderCount || 0), label: 'Pliki, foldery, elementy',
      svgPath: '<path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/>' },
    { icon: 'unique', cls: 'icon-unique', value: stats.UniquePermissionsCount || 0, label: 'Obiekty z unikat. ACL',
      svgPath: '<path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/>' },
    { icon: 'assign', cls: 'icon-assign', value: stats.TotalAssignments || 0, label: 'Przypisania uprawnień',
    ...
    { icon: 'error', cls: 'icon-error', value: stats.ErrorCount || 0, label: 'Błędy skanowania',
      svgPath: '<circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/>' },
    { icon: 'web', cls: 'icon-web', value: stats.TotalObjectsScanned || 0, label: 'Łącznie przeskanowanych',
      svgPath: '<polyline points="22 12 18 12 15 21 9 3 6 12 2 12"/>' },
    { icon: 'list', cls: 'icon-list', value: durationStr, label: 'Czas skanowania',
  ...
  cards.forEach(function(c) {
    ...
    html += '  <div class="stat-card-icon ' + escapeHtml(c.cls) + '">';
    html += '    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">' + c.svgPath + '</svg>';
    html += '  </div>';
    html += '  <div class="stat-card-value">' + escapeHtml(typeof c.value === 'number' ? formatNumber(c.value) : c.value) + '</div>';
    html += '  <div class="stat-card-label">' + escapeHtml(c.label) + '</div>';
    html += '</div>';
  });
  ...
  if (!skipPopulate) {
    document.getElementById('checkAll').addEventListener('change', function() {
      ...
      document.querySelectorAll('#tableBody .row-check').forEach(function(cb) {
        ...
        if (checked) addToSelection(rowId);
        else removeFromSelection(rowId);
      });
    ...
    document.getElementById('btnSelectAll').addEventListener('click', function() {
      document.querySelectorAll('#tableBody .row-check').forEach(function(cb) {
        ...
        addToSelection(cb.getAttribute('data-row-id'));
      });
      updateRemediationPanel();
    });
...
function populateTableBody() {
  ...
  // Event delegation dla checkboxów wierszy
...
function buildTableRow(row) {
  ...
  var principalTypeDisplay = '';
  var principalDisplay = '';
  var permLevelsDisplay = '';
  var sourceDisplay = '';

  if (asgn) {
    principalTypeDisplay = getPrincipalTypeBadge(asgn.PrincipalType);
    ...
    principalDisplay = orphanBadge + adminBadge + '<span title="' + escapeHtml(asgn.LoginName || '') + '">' + escapeHtml(truncateUrl(principalName, 35)) + '</span>';

    var levels = (asgn.PermissionLevels || []).join(', ');
    var limitedBadge = asgn.IsLimitedAccessOnly ? ' <span class="badge badge-limited">LA</span>' : '';
    permLevelsDisplay = escapeHtml(levels) + limitedBadge;

    sourceDisplay = getSourceBadge(asgn.SourceType) + (asgn.SourceName ? ' <span class="text-muted" style="font-size:11px">' + escapeHtml(truncateUrl(asgn.SourceName, 25)) + '</span>' : '');
  } else {
    principalTypeDisplay = '-';
    principalDisplay = '<span class="text-muted">Brak bezpośrednich przypisań (dziedziczenie)</span>';
    permLevelsDisplay = '-';
    sourceDisplay = '<span class="badge badge-inherited">Inherited</span>';
  }

  var actionsHtml = '<div class="action-cell">'
    + '<button class="btn-action" onclick="showObjectDetail(\'' + escapeHtml(obj.ObjectId) + '\')" title="Szczegóły obiektu">'
    + '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>'
    + '</button>'
    + (asgn ? '<button class="btn-action" onclick="addToRemediationFromRow(\'' + escapeHtml(rowId) + '\')" title="Dodaj do remediacji">'
    + '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M14.7 6.3a1 1 0 0 0 0 1.4l1.6 1.6a1 1 0 0 0 1.4 0l3.77-3.77a6 6 0 0 1-7.94 7.94l-6.91 6.91a2.12 2.12 0 0 1-3-3l6.91-6.91a6 6 0 0 1 7.94-7.94l-3.76 3.76z"/></svg>'
    + '</button>' : '')
    + '</div>';

  return '<tr data-row-id="' + escapeHtml(rowId) + '">'
    + '<td class="col-check"><input type="checkbox" class="row-check" data-row-id="' + escapeHtml(rowId) + '"></td>'
    ...
    + '<td>' + principalTypeDisplay + '</td>'
    + '<td class="cell-principal">' + principalDisplay + '</td>'
    + '<td>' + permLevelsDisplay + '</td>'
    + '<td>' + sourceDisplay + '</td>'
    + '<td>' + actionsHtml + '</td>'
    + '</tr>';
}
...
function applyFilters() {
  ...
  App.filteredRows = App.flatRows.filter(function(row) {
    ...
    if (asgn) {
      ...
      if (fPrincipalSearch) {
        ...
        if (login.indexOf(fPrincipalSearch) === -1 &&
            display.indexOf(fPrincipalSearch) === -1 &&
            email.indexOf(fPrincipalSearch) === -1) return false;
      }
    } else {
      if (fPrincipalType || fSourceType || fOnlyDirect || fOnlyLimited || fOnlyOrphaned || fPrincipalSearch) {
        if (fInheritance !== 'inherited' && !fOnlyUnique) {
          return false;
        }
      }
    }
  ...
  // Przebuduj tabelę DataTables
  rebuildTable();
  showToast('info', 'Filtry zastosowane', 'Wyników: ' + App.filteredRows.length);
}
...
function rebuildTable() {
  ...
  if (tbody) {
    var html = '';
    App.filteredRows.forEach(function(row) { html += buildTableRow(row); });
    tbody.innerHTML = html;
  }
...
function addToRemediationFromRow(rowId) {
  ...
  // Zaznacz checkbox
  var cb = document.querySelector('[data-row-id="' + CSS.escape(rowId) + '"]');
  if (cb && cb.classList.contains('row-check')) {
    cb.checked = true;
    var row = cb.closest('tr');
    if (row) row.classList.add('selected');
  }
  ...
  updateRemediationPanel();
  showToast('info', 'Dodano do remediacji', 'Przejdź do zakładki Remediacja.');
}
...
function removeFromSelectionAndUpdate(rowId) {
  removeFromSelection(rowId);
  var cb = document.querySelector('.row-check[data-row-id="' + CSS.escape(rowId) + '"]');
  if (cb) {
    cb.checked = false;
...
function finishSpProgress(results, isDryRun) {
  ...
  summary.innerHTML = '<strong>Podsumowanie [' + mode + ']:</strong> '
    + '<span class="sp-sum-ok">' + ok + ' operacji wykonanych</span>, '
    + (err > 0 ? '<span class="sp-sum-err">' + err + ' błędów</span>' : '<span class="sp-sum-ok">0 błędów</span>');
}
...
function initChangelogTab() {
  ...
  if (!diff || diff.IsFirstReport) {
    ...
    container.innerHTML =
      ...
      '<p>To jest pierwszy raport bazowy. Przy kolejnym skanowaniu ta zakładka pokaże<br>różnice w uprawnieniach względem niniejszego raportu.</p>' +
      '</div>';
    return;
  }
  ...
  html += changelogCard('-' + removed, 'Usunięte obiekty',   '#a4262c', removed > 0 ? '#fff0f0' : '');
  html += changelogCard('~' + changed, 'Zmiany uprawnień',   '#ca5010', changed > 0 ? '#fff8f0' : '');
  html += '</div>';

  if (added === 0 && removed === 0 && changed === 0) {
    html += '<div style="padding:32px;text-align:center;color:var(--text-muted)"><p>✓ Brak zmian względem poprzedniego raportu.</p></div>';
    container.innerHTML = html;
    return;
  }

  // Sekcja: zmiany uprawnień
  if (changed > 0) {
    html += '<div style="padding:0 24px 24px">';
    html += '<h3 style="margin:0 0 12px;padding-top:8px">Zmiany uprawnień (' + changed + ')</h3>';
  ...
  // Sekcja: usunięte obiekty
  if (removed > 0) {
    html += changelogObjectsTable(diff.RemovedObjects || [], 'Usunięte obiekty (' + removed + ')', '#a4262c');
  }
...
function initTableFallback() {
  ...
  if (!tbody) return;
  var html = '';
  App.filteredRows.slice(0, 1000).forEach(function(row) { html += buildTableRow(row); });
  tbody.innerHTML = html;
  if (App.filteredRows.length > 1000) {
